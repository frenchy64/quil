(ns quil.typed.examples.gen-art.19-wave-clock
  (:use quil.core
...
(t/ann mk-lines-stream [-> [-> (U nil (t/Seq Number))]])
(defn mk-lines-stream
  ...
  (let [half-width   (/ (width) 2)
        ...
        lines        (map (t/inst list Number) x1s y1s x2s y2s)]
    (seq->stream lines)))

(t/ann mk-cols-stream [-> [-> (U nil Number)]])
(defn mk-cols-stream
  []
  (let [stroke-cols (cycle-between 0 255)]
    (seq->stream stroke-cols)))

(t/def-alias GenArt19State
  '{:lines-str [-> (U nil (t/Seq Number))]
    :cols-str [-> (U nil Number)]})
...
(t/ann draw [-> Any])
(defn draw []
  (let [lines-str (qt/state GenArt19State :lines-str)
        cols-str  (qt/state GenArt19State :cols-str)
        line-args (lines-str)
        col       (cols-str)]
    (assert col)
    (stroke col 60)
